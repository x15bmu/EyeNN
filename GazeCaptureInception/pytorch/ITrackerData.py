...
class ITrackerData(data.Dataset):
    def __init__(self, split = 'train', imSize=(224,224), gridSize=(25, 25), eyeSize=(299, 299)):
        self.imSize = imSize
        ...
        available_dirs = set(os.listdir(DATASET_PATH))
        for i in range(len(self.metadata['labelRecNum'])):
            rec_num = self.metadata['labelRecNum'][i]
            rec_num_string = '%05d' % rec_num
            if rec_num_string not in available_dirs:
                mask[i] = 0
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            success = torch.LongTensor([1])
        except RuntimeError as e:
            def empty_image():
                return Image.new("RGB", self.imSize, "white")
            success = torch.LongTensor([0])
            imFace = self.transformFace(empty_image())
            imEyeL = self.transformEyeL(empty_image())
            imEyeR = self.transformEyeR(empty_image())
            faceGrid = torch.FloatTensor(self.makeGrid([0, 0, 0, 0]))
            gaze = torch.FloatTensor([1.0, 1.0])

        return success, row, imFace, imEyeL, imEyeR, faceGrid, gaze
